...
for (let i = 0; i < xStartCoords.length; i++) {
    ...
    } else if (Math.abs((xStartCoords[i] - xEndCoords[i])) === Math.abs((yEndCoords[i] - yStartCoords[i]))) {
        let xCoord = xStartCoords[i];
        for (let j = yStartCoords[i], k = 0; yStartCoords[i] >= yEndCoords[i] ? j >= yEndCoords[i] : j <= yEndCoords[i]; yStartCoords[i] >= yEndCoords[i] ? j-- : j++) {
            cartesianPlot[j][xCoord + k]++;
            k += (xStartCoords[i] > xEndCoords[i]) ? -1 : +1;
        }
    }
}
